...
func Setup(dbName string) error {
	...
	tableFiles := []string{"cricket_matches.sql", "cricket_players.sql", "errorlog.sql", "file_mappings.sql", "teams.sql", "venue.sql", "match_stats.sql", "player_match_stats.sql"}

	for _, tableFile := range tableFiles {
		tableName := strings.Replace(tableFile, ".sql", "", -1)
		if !tableExists(Db, tableName) {
			if err := createTable(Db, tableFile); err != nil {
				fmt.Println(err)
				return err
			}
		}
...
func createTable(db *sql.DB, tableName string) error {
	...
	content, err := os.ReadFile("./scripts/tables/" + tableName)
	if err != nil {
		return fmt.Errorf("error reading table file %s: %v", tableName, err)
	}
